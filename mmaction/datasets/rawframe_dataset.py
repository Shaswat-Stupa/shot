...
class RawframeDataset(BaseDataset):
    ...
    def __init__(self,
        ...
        self.filename_tmpl = 'image_{:02}.png'
    ...
    def prepare_train_frames(self, idx):
        ...
        def pipeline_for_a_sample(idx):
            ...
            print(results)
            # """
            # Sample a frame from the frames dir given the sampling strategy.
            ...
            # else:
                # ind = random.randint(0, results['total_frames'] - 1)
            ind = random.randint(0, results['total_frames'] - 1)    
            clipname = self.filename_tmpl.format(ind)

            # if the first char of self.filename_tmpl is a letter, use osp.join;
            # otherwise, directly concat them
            if self.filename_tmpl[0].isalpha():
                filename = osp.join(results['frame_dir'], clipname)
            else:
                filename = results['frame_dir'] + clipname
